...
- Manual `workflow_dispatch` → builds Release with **MinGW** (Qt 6.10.1 `win64_mingw` via `jurplel/install-qt-action@v4` with **Qt-bundled** toolchain `tools: 'tools_mingw1310,qt.tools.win64_mingw1310'` — MinGW 13.1.0, must match the Qt package's ABI; Configure/Build steps resolve it dynamically at `<Qt6_DIR 的父目录>\Tools\mingw*\bin` and prepend to PATH; `-G "MinGW Makefiles"` single-config), then packages: 7z portable + **Inno Setup** installer + release notes, auto-creates tag `vX.Y.Z`, uploads GitHub Release
